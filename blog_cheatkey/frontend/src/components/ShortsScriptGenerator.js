...
const ShortsScriptGenerator = ({ content }) => {
  ...
  // useCallback으로 감싸서 generateScript 함수가 불필요하게 재생성되는 것을 방지합니다.
  const generateScript = useCallback((contentText) => {
    ...
    return shortsScript;
  }, []); // 이 함수는 외부 상태나 props에 의존하지 않으므로 빈 배열을 의존성으로 가집니다.
           // 만약 외부 값에 의존한다면 해당 값을 배열에 추가해야 합니다.

  // 콘텐츠가 변경될 때 스크립트 업데이트
  useEffect(() => {
    if (content) {
      setLoading(true);
      // generateScript 함수가 이제 useCallback으로 메모이즈되었으므로,
      // useEffect의 의존성 배열에 포함시켜도 안전합니다.
      setTimeout(() => {
        setScript(generateScript(content));
        setLoading(false);
      }, 500);
    }
  }, [content, generateScript]); // generateScript를 의존성 배열에 추가
